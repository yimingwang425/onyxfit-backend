...
export const postWriteEntityFiles = asPostWritingEntitiesTask(async function postWriteEntityFiles({ control, application, entities }) {
    for (const entity of (control.filterEntitiesForClient ?? (entities => entities))(entities).filter(entity => !entity.builtInUser)) {
        if (!entity.embedded) {
            ...
            const { entityInstance, entityClass, entityAngularName, entityFolderName, entityFileName, entityUrl, microserviceName, readOnly, entityClassPlural, i18nKeyPrefix, } = entity;
            const pageTitle = (entity.pageTitle ?? enableTranslation) ? `${i18nKeyPrefix}.home.title` : entityClassPlural;
