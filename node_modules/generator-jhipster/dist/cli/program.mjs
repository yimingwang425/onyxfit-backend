...
const buildAllDependencies = async (generatorNames, { env, blueprintNamespaces = [] }) => {
    ...
    const registerDependency = async ({ namespace, blueprintNamespace, }) => {
        ...
        return (await meta?.importModule?.());
    };
...
const addCommandGeneratorOptions = async (command, generatorMeta, { root, blueprintOptionDescription, info } = {}) => {
    ...
    try {
        if (generatorModule.command?.loadGeneratorOptions !== false &&
...
export const buildCommands = ({ program, commands = {}, envBuilder, env, loadCommand = async (key) => {
    const { default: command } = await import(`./${key}.mjs`);
    return command;
}, defaultCommand = GENERATOR_APP, entrypointGenerator, printLogo = printJHipsterLogo, printBlueprintLogo = () => { }, createEnvBuilder, silent, }) => {
    /* create commands */
    Object.entries(commands).forEach(([cmdName, opts]) => {
        ...
        program
            ...
            if (cmdName === 'run') {
                return Promise.all(command.generatorNamespaces.map(generator => env.run(generator, options))).then(results => silent || done(results.find(result => result)), errors => silent || done(errors.find(error => error)));
            }
            if (cmdName === 'upgrade') {
...
export const buildJHipster = async ({ executableName, executableVersion, program = createProgram({ executableName, executableVersion }), blueprints, lookups, createEnvBuilder, envBuilder, commands, devBlueprintPath, env, ...buildOptions } = {}) => {
    createEnvBuilder =
